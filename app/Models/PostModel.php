...
class PostModel extends Model
{
    ...
    protected $allowedFields = ['nama', 'nomor_aset', 'jenis_aset', 'merk', 'NUP', 'kondisi_aset', 'deskripsi', 'lokasi', 'created_at', 'status', 'tanggal_selesai'];
    ...
    public function searchLaporan($search_query)
    {
        return $this->db->table('complaint')
            ...
            ->orLike('merk', $search_query)
            ->orLike('kondisi_aset', $search_query)
    ...
    public function getAllLaporan($start_date = null, $end_date = null)
    {
        ...
        return $builder->select("id, nama, nomor_aset, jenis_aset, merk, nup, kondisi_aset, deskripsi, lokasi, DATE_FORMAT(created_at, '%d-%m-%Y %H:%i:%s') as created_at, status, DATE_FORMAT(tanggal_selesai, '%d-%m-%Y %H:%i:%s') as tanggal_selesai")
            ->get()
            ->getResultArray();
    }
    ...
    {
        return $this->where('status', 'Dalam Proses')->orderBy('created_at', 'DESC')->findAll();
    }
